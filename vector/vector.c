...
static size_t get_new_capacity(size_t target) {
     ...
     */
    printf("vector get new capacity !!!\n");
...
vector *vector_create(copy_constructor_type copy_constructor,
    ...
    if( copy_constructor != NULL && destructor != NULL && default_constructor != NULL ){
        ...
        output->copy_constructor = copy_constructor;
        output->destructor = destructor;
        output->default_constructor = default_constructor;
        output->array = malloc( INITIAL_CAPACITY * sizeof(void*));
        if( output->array == NULL ){
            perror( "error creating vector array" );
            free( output );
            exit( EXIT_FAILURE );
        }
        output->size = 0;
        ...
        output = vector_create( copy_constructor, destructor, default_constructor );
    }
    return output;
...
void vector_resize(vector *this, size_t n) {
    ...
    if( n > this->capacity ){
        ...
        for( size_t i=this->size-1; i>=n; i--)
            this->destructor( this->array[i] );
    }
    this->size = n;
    // your code here
...
void **vector_at(vector *this, size_t position) {
    ...
    if( position < 0 || position >= this->size )
        assert( 0 );
    void** output = this->array + position;
    return output;
}
...
void **vector_back(vector *this) {
    ...
    if( this->size == 0 )
        return NULL;
    void** output = *( this->array + this->size - 1 );
    return output;
}
...
void vector_push_back(vector *this, void *element) {
    ...
    if( this->size == this->capacity ){
        ...
        void** new_array = realloc( this->array, this->capacity * sizeof( void* ) );
        if( new_array == NULL ){
            perror( "error reallocating vector" );
            exit( EXIT_FAILURE );
        }
...
void vector_pop_back(vector *this) {
    ...
    if( this->size > 0){
        ...
        fprintf( stderr, "Warning: Pop on an empty vector.\n" );
}
